// Grassfed pricing page — Just Listed / Just Sold Service
function PricingHero() {
  return (
    <section className="pricing-hero" style={{ padding: '96px 32px 60px', textAlign: 'center' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div className="eyebrow" style={{ marginBottom: 14 }}>Just Listed / Just Sold</div>
        <h1 className="pricing-h1" style={{ fontFamily: 'var(--font-display)', fontSize: 72, fontWeight: 600, letterSpacing: '-0.035em', lineHeight: 1, margin: '0 0 20px', textWrap: 'balance' }}>
          Pay for the<br/><span style={{ color: 'var(--primary)', fontStyle: 'italic' }}>neighborhood,</span> not the seat.
        </h1>
        <p className="pricing-lede" style={{ fontSize: 19, color: 'var(--fg-muted)', lineHeight: 1.5, margin: '0 auto 24px', maxWidth: 620, textWrap: 'pretty' }}>
          One per-campaign fee when an MLS event triggers a send. That's it.
        </p>
        <div className="no-sub-badge" style={{
          display: 'inline-flex', alignItems: 'center', gap: 14,
          padding: '12px 22px', borderRadius: 9999,
          background: 'var(--bg-elevated)', border: '1.5px solid var(--primary)',
...
function PricingTiers() {
  const tiers = [
    { name: 'Starter', sub: '250 nearest homes', emails: '~500 emails', camp: 175, recommended: false,
      bullets: ['MLS auto-trigger', 'Per-home personalization', 'Reply routing to inbox', '1 agent / 1 MLS ID'] },
    { name: 'Growth', sub: '500 nearest homes', emails: '~1,000 emails', camp: 275, recommended: true,
      bullets: ['Everything in Starter', 'A/B subject lines', 'Light-touch follow-up', 'Priority support'] },
    { name: 'Pro', sub: '1,000 nearest homes', emails: '~2,000 emails', camp: 550, recommended: false,
      bullets: ['Everything in Growth', 'Custom sender domain', 'Dedicated sending IP', 'Reply analytics'] },
  ];
  return (
    <section className="pricing-tiers-section" style={{ padding: '0 32px 100px' }}>
      <div className="pricing-tiers-grid" style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
        {tiers.map((t, i) => (
          <div key={i} className={`pricing-tier-card${t.recommended ? ' recommended' : ''}`} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
              <span className="tier-price" style={{ fontFamily: 'var(--font-display)', fontSize: 60, fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1 }}>${t.camp}</span>
...
function CampaignMath() {
  return (
    <section className="campaign-math" style={{ padding: '100px 32px', background: 'var(--dark)', color: 'var(--dark-fg)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div className="eyebrow" style={{ color: 'var(--accent)', marginBottom: 14 }}>The math</div>
        <h2 className="math-h2" style={{ fontSize: 44, margin: '0 0 48px', color: 'var(--dark-fg)', letterSpacing: '-0.025em', lineHeight: 1.1, maxWidth: 720 }}>
          One Growth campaign versus a postcard run.
        </h2>
        <div className="math-compare" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
...
function FAQ() {
  const qs = [
    ['Is there a monthly subscription?', 'No. You pay per campaign, only when an MLS event triggers a send. No monthly fee, no minimum, no term.'],
    ['What if no MLS events match my filters this month?', 'No campaigns fire, no charges. You only pay when a campaign actually sends.'],
    ['Can I cancel anytime?', 'Yes — one click in the Manage Billing tab. No notice period, no cancellation fee. Campaigns already triggered and charged can\'t be cancelled mid-send, but no further charges will occur.'],
    ['What counts as a campaign?', 'One qualifying MLS event = one campaign. Each campaign sends a multi-step email sequence to the nearest-N homes for your tier.'],
    ['What if I close multiple deals in a month?', 'Each one fires a campaign. You can set a Monthly Campaign Cap in your portal to control volume.'],
    ['Do you send from my email address?', 'Starter and Growth send from a Grassfed-managed domain that displays your name. Pro includes a custom sender domain and dedicated IP.'],
  ];
  return (
    <section className="faq-section" style={{ padding: '100px 32px', background: 'var(--bg)' }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        <div className="eyebrow" style={{ marginBottom: 14 }}>FAQ</div>
        <h2 className="faq-h2" style={{ fontSize: 40, margin: '0 0 40px', letterSpacing: '-0.025em' }}>Common questions.</h2>
